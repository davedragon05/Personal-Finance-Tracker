  ...
  Alpine.data('jarvisApp', () => ({
    ...
    init() {
      ...
      document.addEventListener('keydown', e => {
        ...
        if (e.key === 'Escape' && this.activePage === 'calendar') {
    ...
    initCalendar() {
      ...
      const cal = new FullCalendar.Calendar(el, {
        ...
        dateClick: (info) => {
          this.loadDateTransactions(info.dateStr);
        },
        eventSources: [{
    ...
    async loadDateTransactions(dateStr) {
      this.selectedDate = dateStr;
      this.selectedDateLoading = true;
      this.selectedDateTransactions = [];
      const data = await apiFetch(`/api/transactions/?date=${dateStr}&page_size=200`);
      if (Array.isArray(data)) {
        this.selectedDateTransactions = data;
      } else {
        this.selectedDateTransactions = data.results || [];
      }
      this.selectedDateLoading = false;
    },
    ...
    get dayBreakdown() {
      const txns = this.selectedDateTransactions;
      const income = txns.filter(t => t.category === 'INCOME').reduce((s, t) => s + parseFloat(t.amount || 0), 0);
      const expenses = txns.filter(t => ['EXPENSES', 'BILLS', 'SUBSCRIPTIONS'].includes(t.category)).reduce((s, t) => s + parseFloat(t.amount || 0), 0);
      const savings = txns.filter(t => t.category === 'SAVINGS_INVESTMENTS').reduce((s, t) => s + parseFloat(t.amount || 0), 0);
      const debt = txns.filter(t => t.category === 'DEBT').reduce((s, t) => s + parseFloat(t.amount || 0), 0);
      const net = income - expenses - savings - debt;
      return { income, expenses, savings, debt, net };
    },
